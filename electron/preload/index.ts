...
contextBridge.exposeInMainWorld('ipcRenderer', {
  ...
  clearDbHistory: (connectionId: string) => ipcRenderer.invoke('clear-db-history', connectionId),
  /**
   * Terminal
   */
  start_terminal_process: (command: string, projectPath?: string) =>
    ipcRenderer.invoke('start-terminal-process', command, projectPath),
  cancel_terminal_process: () => ipcRenderer.invoke('cancel-terminal-process'),
  /**
   * Redis
   */
  checkRedisStatus: (config: RedisConnection) => ipcRenderer.invoke('check-redis-status', config),
  getRedisDBs: (config: RedisConnection) => ipcRenderer.invoke('get-redis-databases', config),
  getRedisKeys: (
    ...
    cursor?: string,
    count?: number
  ) => ipcRenderer.invoke('get-redis-keys', config, db, pattern, cursor, count),
  getRedisKeyValue: (config: RedisConnection, db: number, key: string, type: string) =>
  ...
  listDatabases: (config: MysqlConnection) => ipcRenderer.invoke('list-databases', config),
   ...
   * Tables
   */
  dropTables: (params: DropTableParams) => ipcRenderer.invoke('drop-tables', params),
  listTables: (config: MysqlConnection) => ipcRenderer.invoke('list-tables', config),
  ...
  getTableRecords: (config: TableRecord) => ipcRenderer.invoke('get-table-records', config),
   ...
   * Project
   */
  saveSettings: (config: Settings) => ipcRenderer.invoke('save-settings', config),
  getSettings: () => ipcRenderer.invoke('get-settings'),
  compareProjectDatabase: (config: { projectPath: string; database: string }) =>
  ...
  readEnvFile: (projectPath: string) => ipcRenderer.invoke('read-env-file', projectPath),
   ...
   * Project Logs
   */
  getProjectLogs: (projectPath: string) => ipcRenderer.invoke('get-project-logs', projectPath),
  ...
  deleteAllLogs: (projectPath: string) => ipcRenderer.invoke('delete-all-logs', projectPath),
  clearAllLogs: (projectPath: string) => ipcRenderer.invoke('clear-all-logs', projectPath),
  /**
   * SQL Executor
   */
