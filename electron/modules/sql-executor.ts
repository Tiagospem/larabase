...
async function executeSqlQueryHandler(_: any, config: MysqlConnection, query: string) {
  let connection: any;
...
async function executeExplainSqlHandler(_: any, config: MysqlConnection, query: string) {
  let connection: any;
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (majorVersion >= 5) {
          try {
            const [jsonResults] = await connection.query(`EXPLAIN FORMAT=JSON ${query}`);
            if (jsonResults && jsonResults[0] && jsonResults[0].EXPLAIN) {
              jsonExplain = jsonResults[0].EXPLAIN;
            }
          } catch (jsonErr) {
            console.log('JSON EXPLAIN not supported or failed:', jsonErr);
          }
        }
      } catch (versionErr) {
