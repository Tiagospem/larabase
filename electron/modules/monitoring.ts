...
async function dropTableTriggers(
	...
	for (const trigger of triggers) {
		try {
			...
			console.error(
				`Error dropping trigger ${trigger}`,
...
async function findAndDropAllTriggers(
	connection: PoolConnection,
	database: string
) {
	try {
		const [triggersResult] = await connection.query(
			SQL.FIND_TRIGGERS_BY_PREFIX,
			[database]
		);

		if (triggersResult && Array.isArray(triggersResult)) {
			for (const row of triggersResult as TriggerRow[]) {
		...
		try {
			const [refTriggers] = await connection.query(
				SQL.FIND_TRIGGERS_BY_REFERENCE,
				[database, `%${ACTIVITY_LOG_TABLE}%`]
			);

			if (refTriggers && Array.isArray(refTriggers)) {
				for (const row of refTriggers as TriggerRow[]) {
					const triggerName = row.TRIGGER_NAME || row.trigger_name;
			...
			console.error(
				`Error dropping triggers referencing ${ACTIVITY_LOG_TABLE}`,
				error instanceof Error ? error.message : ''
			);
		}
		...
		console.error(
			`Error dropping triggers for ${database}`,
			error instanceof Error ? error.message : ''
		);
...
async function createTableTriggers(
	...
	if (!idColumn) {
		try {
			const [firstColumnResult] = await connection.query(
				SQL.GET_FIRST_COLUMN,
				[database, tableName]
			);

			if (
				firstColumnResult &&
				Array.isArray(firstColumnResult) &&
				firstColumnResult.length > 0
			) {
			...
			console.error(
				`Error getting first column for ${tableName}`,
				error instanceof Error ? error.message : ''
			);
		}
	}

	const [allColumnsResult] = await connection.query(SQL.GET_COLUMNS, [
		database,
		tableName
	]);
	...
	const existingColumns = new Set(
		Array.isArray(allColumns)
			? allColumns.map(
					(col: ColumnRow) => col.column_name || col.COLUMN_NAME
				)
			: []
	);
	...
	const oldRecordIdExpr = idColumn
		? `IFNULL(OLD.${idColumn}, '')`
		: `'no-id'`;
  ...
  `;
	...
	try {
		...
		console.error(
			`Error creating triggers for ${tableName}`,
			error instanceof Error ? error.message : ''
		);
		return false;
	}
...
async function dropMonitoringTable(connection: PoolConnection) {
	try {
		...
		console.error(
			'Error dropping monitoring table',
			error instanceof Error ? error.message : ''
		);
		...
		try {
			...
			if (Array.isArray(newActivities) && newActivities.length > 0) {
				state.lastSeenIds.set(
					connectionId,
					(newActivities as ActivityLogRow[])[
	...
	if (state.connections.has(connectionId)) {
		try {
			...
			const windowEntries = Array.from(
				state.windowConnectionMap.entries()
			);
...
export function registerMonitoringHandlers(mainWindow: BrowserWindow) {
	ipcMain.handle(
		...
		async (
			...
			try {
				const connection = await createConnection(appConnection);
				state.connections.set(connectionId, connection);
				state.monitoredDatabases.set(
					connectionId,
					...
					for (const table of tablesResult as TableRow[]) {
						...
						const primaryKeyColumn =
							...
							primaryKeyResult.length > 0
								? (primaryKeyResult[0] as ColumnRow)
										.column_name ||
									(primaryKeyResult[0] as ColumnRow)
										.COLUMN_NAME
								: '';
				...
				try {
					...
					if (Array.isArray(tableExists) && tableExists.length > 0) {
						...
						if (
							...
							state.lastSeenIds.set(
								connectionId,
								(recentActivities as ActivityLogRow[])[0].id
							);
						} else {
							state.lastSeenIds.set(connectionId, 0);
						}
	...
	ipcMain.handle('clear-db-history', async (event, connectionId: string) => {
		...
		try {
			if (!state.connections.has(connectionId)) {
				return {
					success: false,
					message: 'No active monitoring connection found'
				};
			}
			...
			try {
				...
				return {
					success: false,
					message:
						'Connection lost. Please restart monitoring to clear history.',
					connectionLost: true
				};
			}
			...
			return {
				...
				message:
					error instanceof Error
						? error.message
						: 'Failed to clear activity history'
			};
		}
	});
}
