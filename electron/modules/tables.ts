...
function applySortingAndPagination(baseSql: string, connection: any, config: TableRecord) {
  let sql = baseSql;
...
async function getTableStructure(connection: any, database: string, tableName: string) {
...
async function findRestrictingIds(
  ...
  const [constraints]: any[] = await connection.query(REFERENCE_CONSTRAINTS_SQL, [
    tableName,
    connection.database,
  ]);

  const restricts = constraints.filter((c: any) => ['RESTRICT', 'NO ACTION'].includes(c.on_delete));
...
async function listTablesHandler(_: any, config: MysqlConnection) {
  ...
  try {
    ...
    const [tables] = await connection.query(LIST_TABLES_SQL, [config.database]);
    ...
    try {
      const [info] = await connection.query(BASE_COUNT_SQL, [config.database]);
...
async function getTableRecordCountHandler(_: any, config: MysqlConnection, table: Table) {
  let connection: any;
...
async function dropTablesHandler(_: any, params: DropTableParams) {
  ...
  try {
    ...
    return { success: false, message: err.message || 'Error dropping tables' };
  } finally {
    await safeEndConnection(connection);
  }
}

async function truncateTableHandler(_: any, config: MysqlConnection, tableName: string) {
  let connection: any;
  ...
  try {
    ...
    const [fkRes] = await connection.query(HAS_FK_USAGE_SQL, [tableName, config.database]);
    ...
    await connection.query(`TRUNCATE TABLE ${connection.escapeId(tableName)}`);
    if (fkDisabled) {
      await toggleForeignKeyChecks(connection, true);
    }
    ...
    return { success: false, message: err.message || 'Error truncating table' };
  } finally {
    await safeEndConnection(connection);
  }
}
...
async function getTableRecordsHandler(_: any, config: TableRecord) {
  ...
  try {
    ...
    try {
      ...
      if (queryError.code === 'ER_PARSE_ERROR' && queryError.sqlMessage.includes('order')) {
        return {
          success: false,
          message:
...
async function getTableStructureHandler(_: any, config: MysqlConnection, tableName: string) {
  let connection: any;

  try {
    connection = await createConnection(config);

    const structure = await getTableStructure(connection, config.database, tableName);
...
async function getTableForeignKeysHandler(_: any, config: MysqlConnection, tableName: string) {
  let connection: any;
  ...
  try {
    ...
    const [inc] = await connection.query(INCOMING_SQL, [config.database, tableName]);
...
async function deleteTableRecordsHandler(
  _: any,
  { dbConnection: config, tableName, ids, ignoreForeignKeys }: DeleteRowsConfig
) {
  ...
  try {
    ...
    if (ignoreForeignKeys) {
      ...
      const problematic = await findRestrictingIds(connection, tableName, valIds);
    ...
    const { affectedRows } = await deleteRecords(connection, tableName, valIds);
...
async function updateRecordHandler(_: any, config: UpdateTableRecord) {
  ...
  try {
    ...
    const structure = await getTableStructure(connection, config.dbConnection.database, tableName);
    ...
    return { success: false, message: err.message || 'Error updating record' };
  } finally {
    await safeEndConnection(connection);
  }
}

async function getTableIndexesHandler(_: any, config: MysqlConnection, tableName: string) {
  let connection: any;
...
async function getDatabaseSchemaForAIHandler(_: any, config: MysqlConnection) {
  ...
  try {
    ...
    for (const table of tables) {
      ...
      const [incoming] = await connection.query(INCOMING_SQL, [config.database, tableName]);
