...
function registerProjectHandlers(mainWindow: Electron.BrowserWindow) {
  ...
  ipcMain.handle('read-env-file', async (_, projectPath) => {
    try {
      ...
      if (envConfig.DB_PORT) {
        envConfig.DOCKER_INFO = await detectDockerMysql(envConfig.DB_PORT);
      }
  ...
  ipcMain.handle('compare-project-database', async (_, { projectPath, database }) => {
    try {
      const envPath = path.join(projectPath, '.env');
  ...
  ipcMain.handle('find-models-for-tables', async (_, projectPath: string) => {
    try {
      ...
      const resultArray = Object.entries(mapping).map(([table, info]) => ({ table, ...info }));
  ...
  ipcMain.handle('find-factory-files', async (_, projectPath: string, modelName: string) => {
    try {
      ...
      const expectedFactoryNames = [`${modelName}Factory.php`, `${modelName}Factory`];
      ...
      for (const dir of factoryDirs) {
        ...
        const exactFileMatch = factoryFiles.find(
          file =>
            ...
            expectedFactoryNames.some(name => file.name.toLowerCase() === name.toLowerCase())
        );

        if (exactFileMatch) {
          const content = fs.readFileSync(exactFileMatch.path, 'utf8');
        ...
        if (!matchedFactory) {
          for (const file of factoryFiles) {
            ...
            if (classNameMatch && expectedFactoryNames.includes(classNameMatch[1])) {
              matchedFactory = {
                filePath: file.path,
                content: content,
                className: classNameMatch[1],
      ...
      if (matchedFactory) {
        ...
        if (definition) {
          const attributesMatch = definition.match(/return\s*\[\s*([\s\S]*?)\s*];/);
  ...
  ipcMain.handle('find-table-migrations', async (_, projectPath: string, tableName: string) => {
    try {
      ...
      for (const file of migrationFiles) {
        ...
        if (isMainTable || isRenameOperation) {
          ...
          if (
            ...
            content.includes(`Schema::drop("${tableName}"`) ||
            content.includes(`Schema::dropIfExists('${tableName}'`) ||
            content.includes(`Schema::dropIfExists("${tableName}"`)
          ) {
            actions.push({
              type: 'DROP',
  ...
  ipcMain.handle('read-log-file', async (_, filePath, searchTerm = '', logType = '') => {
    try {
      if (!filePath) {
        return {
          success: false,
      ...
      for (const line of lines) {
        ...
        if (dateMatch) {
          ...
          const standardLevelMatch = line.match(/]\s+(\w+)\.([A-Z]+):/);
          ...
          if (
            ...
            level = 'ERROR';
          } else if (uppercaseLevel === 'WARNING' || uppercaseLevel === 'WARN') {
            level = 'WARNING';
          } else if (
            uppercaseLevel === 'INFO' ||
            ...
            level = 'DEBUG';
          } else if (/\bERROR\b|\bERR\b|\bCRIT\b|\bALERT\b|\bEMERG\b|\bFATAL\b/i.test(level)) {
            level = 'ERROR';
      ...
      if (searchTerm) {
        ...
        filteredEntries = filteredEntries.filter(entry =>
          entry.content.some((line: string) => line.toLowerCase().includes(searchLower))
        );
      }

      if (logType && logType !== 'ALL') {
        filteredEntries = filteredEntries.filter(entry => entry.level === logType);
      }
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (dateMatch) {
          ...
          } else if (targetEntryStartIndex >= 0 && targetEntry !== null) {
      ...
      if (targetEntryStartIndex >= 0 && targetEntryEndIndex >= targetEntryStartIndex) {
        entryFound = true;
        for (let i = 0; i < lines.length; i++) {
