		...
		try {
			...
			if (errorOutput && !statusOutput) {
				...
				return {
					success: false,
					message:
						'Error running migration status command: ' +
						errorOutput.split('\n')[0],
					pendingMigrations: [],
					migrationsHistory: []
				};
				...
				if (isTableFormat) {
					...
					if (line.includes('| Yes ')) {
						...
						if (match && match[1]) {
							...
							if (
								migrationName &&
								!migrationsHistory.includes(migrationName)
							) {
								migrationsHistory.push(migrationName);
							}
						}
			...
			if (migrationsHistory.length === 0) {
				...
				try {
					...
					console.error(
						'Error getting migrations from database:',
						dbError
					);
