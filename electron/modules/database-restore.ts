...
import { createConnection, testConnection, releaseConnection } from '../helpers/mysql';
import { executeMysqlFileInContainer, createDockerClient } from '../helpers/docker';
...
async function validateDatabaseHasContent(restorationObject: RestoreConfig) {
  ...
  try {
    try {
      dbConnection = await createConnection(restorationObject.project.db_config, {
...
function buildDockerRestoreCommand(restorationObject: RestoreConfig) {
  ...
  console.log(`Docker restore: Will restore to database "${restorationObject.targetDatabase}"`);
                    ...
                    if (activeRestoreProcess) {
                      ...
                      if (
                        ...
                        try {
                          ...
                          exec
                            .stop()
                            .catch((err: any) => console.error('Error stopping Docker exec:', err));
                        } catch (dockerErr) {
...
async function extractTables(filePath: string, isGzipped: boolean) {
  return new Promise<TableInfo[]>((resolve, reject) => {
    let stream: Readable = fs.createReadStream(filePath, { highWaterMark: 64 * 1024 });
...
function registerDatabaseRestoreHandlers(mainWindow: BrowserWindow) {
  ...
  ipcMain.handle('extract-tables-from-sql', async (_, filePath) => {
    try {
      ...
      const tables: TableInfo[] = await extractTables(filePath, isGzipped);
  ...
  ipcMain.handle('restore-database', async (_, restorationConfig: RestoreConfig) => {
    try {
      if (
        !restorationConfig ||
        ...
        return {
          success: false,
          message: 'Missing required parameters (connection or filePath)',
        };
      }
      ...
      const targetDatabase =
        restorationConfig.targetDatabase || restorationConfig.project.db_config.database;
      ...
      if (restorationConfig.ignoredTables && restorationConfig.ignoredTables.length > 0) {
        console.log(`Will ignore these tables: ${restorationConfig.ignoredTables.join(', ')}`);
      }

      try {
        const isGzipped = restorationConfig.filePath.toLowerCase().endsWith('.gz');
        const useDocker = project.usingSail || (project.dockerInfo && project.dockerInfo.isDocker);

        console.log(`Restoring database: ${targetDatabase}`);
        console.log(`Original connection database: ${project.db_config.database}`);
        console.log(`Docker mode: ${useDocker ? 'Yes' : 'No'}`);
        console.log(`Gzipped file: ${isGzipped ? 'Yes' : 'No'}`);
        console.log(`Ignored tables: ${restorationConfig.ignoredTables.length}`);
        ...
        if (global.currentRestoreConfig && global.currentRestoreConfig.container) {
          try {
            ...
            container
              ...
              .catch(() =>
                console.log('Failed to send MySQL kill command (expected if MySQL is already down)')
              );
          } catch (err) {
