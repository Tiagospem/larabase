...
function createRedisClient(config: RedisConnection) {
  return new Redis({
    host: config.host,
    port: typeof config.port === 'string' ? parseInt(config.port, 10) : (config.port ?? 6379),
...
function registerRedisHandlers() {
  ...
  ipcMain.handle('get-redis-databases', async (_, config: RedisConnection) => {
    const client = createRedisClient(config);
  ...
  ipcMain.handle(
    'get-redis-key-value',
    async (_, config: RedisConnection, db: number, key: string, type: string) => {
      const client = createRedisClient(config);
  ...
  ipcMain.handle('flush-redis-db', async (_, config: RedisConnection, db: number) => {
    const client = createRedisClient(config);
