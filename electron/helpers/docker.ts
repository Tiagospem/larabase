...
async function detectDockerMysql(port: number) {
	const result = {
	...
	try {
		...
		for (const container of containers) {
			...
			if (container.Ports && container.Ports.length > 0) {
				const portMatch = container.Ports.some(
					(p) => p.PublicPort == port && p.PrivatePort == 3306
				);
		...
		result.message =
			result.message || `No MySQL Docker container found on port ${port}`;
		return result;
	} catch (error) {
...
async function executeMysqlFileInContainer(
	restorationConfig: RestorationProgressConfig,
	progressCallback: Function,
	ignoreCreateDatabase = true
) {
	let isCancelled = false;

	const safeProgressCallback = (progress: number) => {
		if (
			isCancelled ||
	...
	const sqlFilePath = restorationConfig.sqlFilePath;
	const containerName = restorationConfig.container;
	const ignoredTables = restorationConfig.ignoredTables;
	const credentials = restorationConfig.connection;
	const database = restorationConfig.targetDatabase;
	...
	try {
		...
		const hasFilters =
			Array.isArray(ignoredTables) && ignoredTables.length > 0;
		...
		return new Promise((resolve, reject) => {
			try {
				let processStream: fs.ReadStream | zlib.Gunzip;
				...
				if (hasFilters || ignoreCreateDatabase) {
					...
					let batchSize = 0;
					const maxBatchSize = 512 * 1024;

					const ignoredTablePatterns = ignoredTables.map(
						(table) =>
							new RegExp(`INSERT\\s+INTO\\s+\`?${table}\`?`, 'i')
					);
					const createDatabasePattern = new RegExp(
						`CREATE\\s+DATABASE|USE\\s+\``,
						'i'
					);
					...
					const processBatch = () => {
						if (buffer.length > 0) {
							...
							for (const line of lines) {
								...
								if (!inIgnoredInsert) {
									const shouldIgnoreTable =
										ignoredTablePatterns.some((pattern) =>
											pattern.test(line)
										);
									const shouldIgnoreDbCommand =
										createDatabasePattern &&
										createDatabasePattern.test(line);
		...
		console.error(
			'Error executing MySQL file in container:',
			error.message
		);
