...
async function detectDockerMysql(port: number) {
  ...
  try {
    ...
    for (const container of containers) {
      ...
      if (container.Ports && container.Ports.length > 0) {
        const portMatch = container.Ports.some(p => p.PublicPort == port && p.PrivatePort == 3306);
...
async function executeMysqlFileInContainer(
  ...
  const safeProgressCallback = (progress: number) => {
    if (isCancelled || global.cancelRestoreRequested || !global.restoreInProgress) {
      return;
    }
  ...
  try {
    ...
    const execStream = (await exec.start({ hijack: true, stdin: true })) as DockerExecStream;
      ...
      try {
        ...
        if (hasFilters || ignoreCreateDatabase) {
          ...
          const ignoredTablePatterns = ignoredTables.map(
            table => new RegExp(`INSERT\\s+INTO\\s+\`?${table}\`?`, 'i')
          );
          ...
          const processBatch = () => {
            if (buffer.length > 0) {
              ...
              for (const line of lines) {
                ...
                if (!inIgnoredInsert) {
                  const shouldIgnoreTable = ignoredTablePatterns.some(pattern =>
                    pattern.test(line)
                  );
                  const shouldIgnoreDbCommand =
                    createDatabasePattern && createDatabasePattern.test(line);
