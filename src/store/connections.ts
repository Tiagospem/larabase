...
export const useConnectionsStore = defineStore('connections', () => {
        ...
        try {
          ...
          if (savedProjects && Array.isArray(savedProjects) && savedProjects.length > 0) {
            for (const project of savedProjects) {
              const check = await window.ipcRenderer.testMySQLConnection(project.db_config);
  ...
  async function removeConnection(projectId: string) {
    const localStorageConnections = JSON.parse(localStorage.getItem('connections') || '[]');
  ...
  const getSelectedProject = computed(
    () => connections.value.find(c => c.id === currentProjectId.value) || null
  );
