  ...
  return defineStore(table, () => {
    ...
    const columns = computed<TableColumn[]>(() => {
      if (tableStructure.value.length > 0) {
        ...
        const idIndex = newColumns.findIndex((col: TableColumn) => col.field === 'id');
      ...
      if (keys.length > 0) {
        ...
        const idIndex = newColumns.findIndex(col => col.field === 'id');
    ...
    async function getTableForeignKeys(tableName: string) {
      ...
      try {
        const { success, foreignKeys, message } = await window.ipcRenderer.getTableForeignKeys(
          toRaw(project?.db_config),
          tableName
        );

        if (!success) {
          console.error('Error getting foreign keys for table:', tableName, message);
          return [];
        }
    ...
    async function getTableData(tableName: string) {
      try {
        ...
        if (response.success) {
          ...
          tableData.value = response.data.map((row: any, index: number) => {
            if (row.id === undefined) {
