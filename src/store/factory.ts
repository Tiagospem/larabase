  ...
  return defineStore(`factory-${tableName}`, () => {
    ...
    const selectedAttributes = computed(() => {
      ...
      return factory.value.attributes
        ...
        .map((attr: FactoryAttribute) => {
          return {
            name: attr.name,
            value: attr.customValue !== undefined ? attr.customValue : attr.value,
          };
        });
    });
    ...
    const generateSeedCommand = computed(() => {
      ...
      if (attributes.length > 0) {
        const stateValues = attributes.map(attr => `'${attr.name}' => ${attr.value}`).join(', ');
    ...
    async function loadFactory() {
      ...
      try {
        ...
        if (projectStore.models && projectStore.models.models) {
          const matchingModel = projectStore.models.models.find(m => m.table === tableName);
          ...
          if (result.success && result.factory) {
            ...
            if (factoryData.attributes) {
              factoryData.attributes = factoryData.attributes.map((attr: FactoryAttribute) => ({
                ...attr,
                selected: false,
              }));
            }
    ...
    function toggleAttributeSelection(attrName: string) {
      ...
      const attr = factory.value.attributes.find(a => a.name === attrName);
      if (attr) {
