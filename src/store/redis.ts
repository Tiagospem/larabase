...
export const useRedisStore = defineStore('redis', () => {
  ...
  async function checkRedisAvailability(project: ProjectConnection) {
    ...
    try {
      ...
      const result = await window.ipcRenderer.checkRedisStatus(toRaw(config));
  ...
  async function fetchDatabases() {
    ...
    try {
      const result = await window.ipcRenderer.getRedisDBs(toRaw(currentConnection.value));
  ...
  async function deleteKey(key: string) {
    ...
    try {
      ...
      if (result.success) {
        ...
        const dbIndex = databases.value.findIndex(db => db.id === selectedDb.value);
