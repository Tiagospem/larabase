...
export const useDatabaseStore = defineStore('database', () => {
  ...
  function updateLocalStorageTables(
    ...
    try {
      ...
      if (tableName) {
        updateSingleTable(parsedData.tables, tableName, newCount, isApproximate);
      } else {
  ...
  async function loadTables(project: ProjectConnection) {
    ...
    try {
      ...
      try {
        ...
        if (storedData) {
          ...
          if (parsedData && parsedData.tables) {
            const cacheExpired = !parsedData.timestamp || Date.now() - parsedData.timestamp > 10000;
      ...
      const result = await window.ipcRenderer.listTables(databaseConnection);
  ...
  async function truncateTable(connection: MysqlConnection, tableName: string) {
    const result = await window.ipcRenderer.truncateTable(toRaw(connection), tableName);
  ...
  async function truncateTables(tableNames: string[]) {
    ...
    const results = await Promise.all(
      tableNames.map(async tableName => {
        try {
          const result = await truncateTable(project.db_config, tableName);
          return {
            tableName,
  ...
  async function deleteTableRecords(
    ...
    const getTable = sidebarStore.localTables.find(t => t.name === tableName);
    ...
    if (getTable) {
      ...
      if (countResult && countResult.success) {
        sidebarStore.updateTableRecordCount(tableName, countResult.count);
      }
    }
