...
export const useProjectStore = defineStore('project', () => {
  ...
  const targetDatabase = computed(() => selectedProject.value?.db_config.database || '');
  ...
  async function getModelsForTables(project: ProjectConnection) {
    ...
    try {
      models.value = await window.ipcRenderer.findModelsForTables(project.projectPath);
  ...
  async function checkProjectDatabase() {
    ...
    try {
      const { success, isMatch, projectDatabase } = await window.ipcRenderer.compareProjectDatabase(
        { projectPath: path, database: targetDatabase.value }
      );
