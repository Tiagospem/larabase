...
export const useSqlScratchesStore = defineStore('sqlScratches', () => {
  ...
  function loadScratches(projectId: string) {
    try {
      const savedScratches = localStorage.getItem(`sql-scratches-${projectId}`);
      if (savedScratches) {
        scratches.value = JSON.parse(savedScratches).filter(
          (scratch: SqlScratch) => scratch.projectId === projectId
        );

        const savedActiveScratchId = localStorage.getItem(`sql-active-scratch-${projectId}`);

        if (savedActiveScratchId && scratches.value.some(s => s.id === savedActiveScratchId)) {
          activeScratchId.value = savedActiveScratchId;
        } else {
          const defaultScratch = scratches.value.find(
            scratch => scratch.isDefault && scratch.projectId === projectId
          );
  ...
  function saveScratches(projectId: string) {
    try {
      const projectScratches = scratches.value.filter(scratch => scratch.projectId === projectId);
      localStorage.setItem(`sql-scratches-${projectId}`, JSON.stringify(projectScratches));
    } catch (error) {
  ...
  function saveActiveScratchId(projectId: string) {
    if (activeScratchId.value) {
      localStorage.setItem(`sql-active-scratch-${projectId}`, activeScratchId.value);
    }
  }
  ...
  function updateScratch(id: string, data: Partial<SqlScratch>, projectId: string) {
  ...
  function deleteScratch(id: string, projectId: string) {
    ...
    if (index !== -1) {
      ...
      if (activeScratchId.value === id) {
        const defaultScratch = scratches.value.find(s => s.isDefault && s.projectId === projectId);
        if (defaultScratch) {
          activeScratchId.value = defaultScratch.id;
          saveActiveScratchId(projectId);
        }
  ...
  function getActiveScratch() {
    ...
    return scratches.value.find(scratch => scratch.id === activeScratchId.value);
  }
