...
export function useSQLEditor(props: { modelValue: string }, emit: EmitFn) {
  ...
  const { databaseSchema, fetchDatabaseSchema, initializeSchema } = useDatabaseSchema();
  ...
  const getSchemaForAutocompletion = () => {
    ...
    databaseSchema.value.tables.forEach(table => {
      if (table.columns && table.columns.length > 0) {
        columnsByTable[table.name] = table.columns.map(col => col.name);
  ...
  const registerSQLCompletionProvider = () => {
    const { tables, columnsByTable, columnMetadata } = getSchemaForAutocompletion();
    ...
    monaco.languages.registerCompletionItemProvider('sql', {
      ...
      provideCompletionItems: (model, position) => {
        ...
        const fromMatch = textUntilPosition.match(/\b(FROM|JOIN)\s+(\w*)$/i);
        if (fromMatch) {
          tables.forEach(table => {
            const tableInfo = databaseSchema.value?.tables.find(t => t.name === table);
            ...
            suggestions.push({
              ...
              range: {
                startLineNumber: position.lineNumber,
                endLineNumber: position.lineNumber,
                startColumn: position.column - (fromMatch[2]?.length || 0),
                endColumn: position.column,
              },
            });
          });
        ...
        if (tableColumnMatch) {
          ...
          columns.forEach(column => {
            ...
            const keyText = keyInfo.length ? ` [${keyInfo.join(', ')}]` : '';
        ...
        if (
          ...
          Object.entries(columnsByTable).forEach(([table, columns]) => {
            columns.forEach(column => {
              const metadata = columnMetadata[table]?.[column];
              const keyInfo = [];
    ...
    editor.addAction({
      ...
      run: async function (ed) {
        ...
        if (selectedText.trim()) {
          const projectConfig = connectionsStore.getSelectedProject?.db_config;
          if (projectConfig) {
            sqlResultsStore.isLoading = true;
            const startTime = Date.now();
            try {
              const result = await window.ipcRenderer.executeSqlQuery(
                toRaw(projectConfig),
                selectedText.trim()
              );
              ...
              if (result && result.success) {
                sqlResultsStore.setResults(result.results, selectedText.trim());
                sqlResultsStore.lastQueryTime = executionTime;
              } else {
                const errorMsg =
                  result && result.error ? result.error : 'Unknown SQL error occurred';
                console.error('SQL Error:', errorMsg);
                sqlResultsStore.setError(`Error executing query: ${errorMsg}`);
              }
            } catch (error) {
              console.error('Exception executing SQL:', error);
              sqlResultsStore.setError(
                error instanceof Error ? error.message : 'Error executing SQL query'
              );
            } finally {
      ...
      run: async function (ed) {
        ...
        if (selectedText.trim()) {
          ...
          if (projectConfig) {
            ...
            try {
              const result = await window.ipcRenderer.executeExplainSql(
                toRaw(projectConfig),
                query
              );
        ...
        try {
          ...
          if (selection && !selection.isEmpty()) {
            ...
            if (model) {
              ...
              range = new monaco.Range(1, 1, lastLineNumber, lastLineLength + 1);
            } else {
              return;
            }
          }
      ...
      editor.addAction({
        ...
        run: async function (ed) {
          ...
          try {
            ...
            const result = await aiService.fixSQLQuery(selectedText, language);
            ...
            alert('Error fixing SQL: ' + (error instanceof Error ? error.message : String(error)));
          } finally {
