...
export function useForeignKeyNavigation() {
	...
	function isForeignKeyColumn(
		...
		return foreignKeys.some(
			(fk) => fk.column === columnName && fk.type === 'outgoing'
		);
	}
	...
	async function navigateToForeignKey(
		...
		if (value === null || value === undefined) {
			if (onError)
				onError(
					'Null or undefined value. Unable to navigate to the related record.'
				);
			return false;
		}
		...
		if (foreignKeys.length === 0) {
			if (onError)
				onError('Could not retrieve foreign keys for this table');
			return false;
		}
		...
		if (!isForeignKeyColumn(column, foreignKeys)) {
			if (onError)
				onError(
					`Foreign key information not found for column "${column}"`
				);
			return false;
		}
		...
		if (!targetTable || !targetColumn) {
			if (onError)
				onError('Referenced table or column not found in foreign key');
			return false;
		}
		...
		const findTable = sidebarStore.localTables.find(
		...
		if (onError)
			onError(`Target table "${targetTable}" not found in the database`);
		return false;
	}
