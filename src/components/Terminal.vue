...
`;
    ...
    if (terminal.value) {
      ...
      try {
        if (window.ipcRenderer) {
          ...
          window.ipcRenderer.start_terminal_process(savedCommand, projectPath.value);
        } else {
  ...
  onMounted(() => {
    ...
    if (terminalElement.value) {
      ...
      term.writeln('Terminal started. Type commands in the input field below.');
      term.writeln(`Working directory: ${projectPath.value || 'Application Root'}`);
      term.writeln('Commands will be displayed in real-time as they execute.');
      term.writeln('');
...
<template>
  <div class="terminal-container">
    ...
    <div class="terminal-panel text-base-content" :class="{ visible: isVisible }">
      <div class="terminal-header bg-base-200">
        ...
        <div class="terminal-actions">
          ...
          <button class="action-button" @click="clearTerminal" title="Clear terminal">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
          ...
          <button class="close-button" @click="toggleTerminal">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
      ...
      <div class="terminal-body">
        <div ref="terminalElement" class="terminal-output"></div>
        <div class="command-input-container bg-base-200 border-base-content/20 border-t">
          <span class="prompt text-primary" :class="{ loading: isCommandRunning }">
            {{ isCommandRunning ? '⟳' : '$' }}
          </span>
          <input
            id="command-input"
