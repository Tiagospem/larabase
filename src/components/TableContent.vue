<script setup lang="ts">
  ...
  const DataTab = markRaw(defineAsyncComponent(() => import('./database/tables/Records.vue')));
  ...
  const ModelTab = markRaw(defineAsyncComponent(() => import('./database/tables/ModelTab.vue')));
...
<template>
  <div class="flex h-full flex-col">
    <div class="bg-base-300 flex items-center border-b border-black/10 px-2 py-1">
      <div class="flex gap-1">
        <a
          ...
          :class="{
            ...
            'hover:text-primary opacity-50': activeContentTab !== tab.id,
            'pointer-events-none animate-pulse opacity-20': sidebarStore.isLoading,
          }"
    ...
    <keep-alive>
      <component :is="currentTabComponent" :tableName="props.tableName" :key="activeContentTab" />
    </keep-alive>
  </div>
</template>
