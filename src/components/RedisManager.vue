<script setup lang="ts">
  ...
  watchEffect(() => {
    ...
    if (redisStore.selectedDb !== null) {
      searchTimeout = setTimeout(async () => {
        await redisStore.fetchKeys(searchTerm.value ? `*${searchTerm.value}*` : '*', true);
      }, 300);
    }
  });
  ...
  async function flushAllDatabases() {
    if (
      !confirm('Are you sure you want to flush ALL Redis databases? This action cannot be undone.')
    ) {
      return;
    }
...
<template>
  <Modal
    ...
    :show-action-button="redisStore.isRedisAvailable && redisStore.databases.length > 0"
    :is-loading-action="isFlushingAll"
    :show-cancel-button="false"
  >
    <div v-if="redisStore.isRedisAvailable" class="flex h-[50vh] flex-col px-1">
      <div class="flex flex-1 overflow-hidden">
        <div class="h-full w-1/4 overflow-hidden border-r pr-3">
          <div class="mb-3 flex items-center justify-between px-1">
            ...
            >
              <span v-if="isRefreshingDbs" class="loading loading-spinner loading-xs"></span>
              <svg
                v-else
          ...
          <div class="h-[calc(100%-2rem)]">
            <RedisDatabaseList @select-database="switchTab('keys')" />
          </div>
        </div>

        <div class="flex h-full w-3/4 flex-col">
          <div v-if="redisStore.selectedDb !== null" class="mb-4 px-1">
            <div class="flex items-center">
              ...
              >
                <span v-if="isRefreshingKeys" class="loading loading-spinner loading-xs"></span>
                <svg
                  v-else
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-4 w-4"
          ...
          <div class="flex-1 overflow-hidden">
            <div v-if="redisStore.selectedDb !== null" class="flex h-full">
              <div class="h-full w-2/5 overflow-hidden border-r pr-2">
                <RedisKeysList />
              </div>
            ...
            <div v-else class="flex h-full w-full items-center justify-center">
              <div class="text-center text-sm">
                ...
                <p class="mt-2">Select a database to view keys</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-else class="flex h-40 items-center justify-center">
      <div class="text-center">
        ...
        <p class="text-sm">Check your Redis configuration in the project settings.</p>
      </div>
    </div>
  </Modal>
</template>
