...
const filteredScratches = computed(() => {
	...
	return scratchService.scratches.filter((scratch) =>
		scratch.name.toLowerCase().includes(query)
	);
});
	...
	if (scratch) {
		if (window.sqlEditorContent) {
			...
			localStorage.setItem(
				`sql-editor-content-${props.projectId}`,
				scratch.content
			);
		}
...
function createNewScratch() {
	if (newScratchName.value.trim()) {
		...
		if (window.sqlEditorContent) {
			...
			editorContent =
				localStorage.getItem(`sql-editor-content-${props.projectId}`) ||
				'';
		}
...
<template>
	...
	>
		...
		<div class="border-base-300 border-b p-4">
			<div class="mb-4">
				<label
					class="text-base-content/70 mb-1 block text-xs font-medium"
		...
		<div class="flex-1 overflow-auto p-2">
			<div
				v-if="filteredScratches.length === 0"
				class="mt-4 text-center text-sm opacity-70"
			>
				...
				<div
					...
					:class="{
						'border-primary border-2':
							scratch.id === activeScratch?.id,
						'opacity-70': scratch.isDefault
					}"
				>
					<div
						v-if="editingScratchId === scratch.id"
					...
					>
						...
						<div class="flex justify-end gap-1">
							<button
								@click="cancelEditScratch"
								class="btn btn-ghost btn-xs"
							>
								Cancel
							</button>
							<button
								@click="saveEditScratch"
