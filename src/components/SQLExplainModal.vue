...
const hasExplainResults = computed(() => {
	return (
		props.explainData &&
		props.explainData.rawExplain &&
		props.explainData.rawExplain.length > 0
	);
});
...
const generateAIAnalysis = async () => {
	if (
		...
		!hasExplainResults.value ||
		isLoadingAIAnalysis.value
	)
		return;
	...
	try {
		...
		const explainText = JSON.stringify(
			props.explainData.rawExplain,
			null,
			2
		);
...
`;
		...
		const result = await aiService.analyzeSQLExplain(
			analyzePrompt,
			language
		);
...
const formatAIAnalysis = (text: string): string => {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (line.trim().startsWith('#')) {
			...
			if (level === 1) {
				formattedLines.push(
					`<h2 class="text-md font-bold mt-4 mb-3">${title}</h2>`
				);
			} else if (level === 2) {
				formattedLines.push(
					`<h3 class="text-sm font-semibold mt-3 mb-2">${title}</h3>`
				);
			} else {
				formattedLines.push(
					`<h4 class="text-sm font-medium mt-2 mb-1">${title}</h4>`
				);
			}
			continue;
		}
...
const isProblematicRow = (row: any): boolean => {
	...
	if (row.Extra && typeof row.Extra === 'string') {
		if (
			row.Extra.includes('Using filesort') ||
			row.Extra.includes('Using temporary')
		) {
			return true;
		}
	}
...
const isProblematicValue = (key: any, value: any): boolean => {
	...
	switch (keyString.toLowerCase()) {
		...
		case 'extra':
			return (
				stringValue.includes('Using filesort') ||
				stringValue.includes('Using temporary')
			);
...
<template>
	...
	>
		<div class="space-y-6 max-h-[80vh]">
				...
				<div class="bg-base-300 relative rounded-md p-3">
					<pre
						...
						:class="{
							'max-h-40':
								!isQueryExpanded &&
								explainData?.queryToExplain?.length > 150
						}"
						>{{ queryPreview }}</pre
					>
				</div>
			...
			<div
				v-else-if="explainData.error"
				class="bg-base-100 alert alert-error rounded-md p-4"
			>
				<svg
			...
			>
				<div class="mb-3 flex items-center justify-between">
					...
					<button
						@click="showExplainHelp = !showExplainHelp"
						class="btn btn-xs"
					>
						{{
							showExplainHelp
				...
				<div
					v-if="showExplainHelp"
					class="bg-base-200 mb-4 rounded-md p-3 text-sm"
				>
					...
					<ul class="list-disc space-y-1 pl-5">
						<li>
							<strong>type</strong>: Join type, ordered from best
							to worst: system, const, eq_ref, ref, range, index,
							ALL
						</li>
						<li>
							<strong>key</strong>: The index MySQL actually
							decided to use
						</li>
						<li>
							<strong>rows</strong>: Estimated number of rows that
							need to be examined
						</li>
						<li>
							<strong>filtered</strong>: Estimated percentage of
							rows that will be filtered by conditions
						</li>
						<li>
							<strong>Extra</strong>: Additional information about
							how MySQL executes the query
						</li>
					</ul>
					...
					<ul class="list-disc space-y-1 pl-5">
						<li>
							<strong>Using filesort</strong>: MySQL needs an
							extra pass to sort results, potentially slow
						</li>
						<li>
							<strong>Using temporary</strong>: MySQL needs to
							create a temporary table to hold results
						</li>
						<li>
							<strong>Using where</strong>: A WHERE clause is used
							to filter results
						</li>
						<li>
							<strong>ALL</strong> in type column: Full table
							scan, usually inefficient
						</li>
						<li>
							Large numbers in <strong>rows</strong> column: Many
							rows need to be examined
						</li>
					</ul>
					...
					<ul class="list-disc space-y-1 pl-5">
						<li>
							Add indexes for columns used in JOIN, WHERE, ORDER
							BY, and GROUP BY clauses
						</li>
						<li>Use ANALYZE TABLE to update table statistics</li>
						<li>
							Ensure columns being compared have the same type and
							size
						</li>
						<li>
							Avoid full table scans (type: ALL) when possible
						</li>
					</ul>
				</div>
				...
				<div class="border-base-300 overflow-hidden rounded-md border">
					<div class="h-[400px] overflow-auto">
						<table
							class="table-compact table-xs w-full border-collapse"
						>
							<thead
								class="bg-base-300 sticky top-0 z-10 shadow-md"
							>
								<tr>
									<th
										v-for="(_, key) in explainData
											.rawExplain[0]"
										:key="key"
							...
							<tbody>
								<tr
									v-for="(
										row, index
									) in explainData.rawExplain"
									:key="index"
									:class="{
										'bg-error bg-opacity-20 text-white':
											isProblematicRow(row)
									}"
					...
					<div
						class="bg-base-300 flex items-center px-2 py-1 text-xs opacity-80"
					>
						<div class="bg-error mr-1 h-3 w-3 rounded-full"></div>
						<span
							>Highlighted items indicate potential performance
							issues</span
						>
					</div>
				</div>
			...
			<div
				v-else
				class="bg-base-100 alert alert-info border-base-300 rounded-md border p-4"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					fill="none"
			...
			>
				...
				<div
					v-if="isLoadingAIAnalysis"
					class="flex flex-col items-center justify-center py-6"
				>
					<span
						class="loading loading-spinner loading-md mb-2"
					></span>
				...
				<div
					...
					class="bg-base-300 prose prose-sm max-w-none rounded-md p-4"
				>
					<div
						v-html="formatAIAnalysis(aiAnalysis)"
				...
				>
					<p>
						Configure an AI provider in settings to get intelligent
						optimization suggestions for your query.
					</p>
				</div>

				<div
					v-else-if="
						hasExplainResults && !isLoadingAIAnalysis && !aiAnalysis
					"
					class="bg-base-300 rounded-md p-4 text-sm"
				>
					<p>
						Click "Request AI Analysis" to get optimization
						suggestions for your query.
					</p>
				</div>
			</div>
		</div>
