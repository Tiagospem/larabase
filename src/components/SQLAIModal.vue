...
<template>
  ...
  >
    <div class="space-y-4">
      <div>
        <label class="label">
          <span class="label-text">What SQL query would you like to generate?</span>
        </label>
        <textarea
          v-model="prompt"
      ...
      <div class="flex justify-end">
        <button class="btn btn-primary" @click="generateSql" :disabled="isDisabled">
      ...
      <div v-if="response" class="space-y-2">
        ...
        <div class="bg-base-300 max-h-96 overflow-auto rounded-lg p-4 text-sm">
          <pre class="break-words whitespace-pre-wrap"><code>{{ response }}</code></pre>
        </div>
        <div class="mt-2 flex justify-end">
          <button class="btn btn-primary" @click="applySql">Apply SQL</button>
        </div>
      </div>
    </div>
  </Modal>
