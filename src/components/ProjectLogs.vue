<script setup lang="ts">
      ...
      if (logToDelete.value) {
        ...
        if (result && result.success) {
          ...
          if (logsStore.selectedLogFile) {
            await logsStore.loadLogFileContent(logsStore.selectedLogFile.path);
          }
        } else {
          showAlert(`Failed to delete log entry: ${result?.message || 'Unknown error'}`, 'error');
        }
      }
    } catch (error: any) {
  ...
  async function confirmDeleteAllLogs() {
    try {
      ...
      if (result && result.success) {
        ...
        showAlert(`Failed to clear logs: ${result?.message || 'Unknown error'}`, 'error');
      }
    } catch (error: any) {
  ...
  async function confirmDeleteFile() {
    try {
      ...
      const result = await window.ipcRenderer.deleteLogFile(logsStore.selectedLogFile.path);
...
<template>
  ...
  >
    <div class="flex flex-col">
      <div class="flex justify-end px-4 py-2">
        <button class="btn btn-sm mr-2" @click="refreshLogs" :disabled="refreshing">
          <span v-if="refreshing" class="loading loading-spinner loading-xs mr-1"></span>
          <svg
            v-else
      ...
      <div class="bg-base-200 m-2 grid grid-cols-1 gap-4 rounded-md p-4 md:grid-cols-3">
        <div class="flex flex-col gap-1">
          ...
          >
            <option v-if="logsStore.logFiles.length === 0" disabled value="">
              No log files found
            </option>
            <option
        ...
        <div class="flex flex-col gap-1">
          ...
          <select
            ...
            @change="logsStore.filterLogs(false)"
          >
            <option v-for="type in logsStore.availableLogTypes" :key="type" :value="type">
              {{ type }}
            </option>
          </select>
        </div>
      ...
      <div class="overflow-y-hidden">
        <div v-if="logsStore.isLoading" class="flex h-full w-full items-center justify-center">
          <div class="loading loading-spinner loading-lg text-primary"></div>
        </div>

        <div v-else-if="logsStore.error" class="flex h-full w-full items-center justify-center">
          <div class="text-center">
            <h3 class="text-error text-lg font-medium">{{ logsStore.error }}</h3>
          </div>
        </div>

        <div
          v-else-if="!logsStore.logEntries || logsStore.logEntries.length === 0"
          class="flex h-full w-full items-center justify-center"
        >
          <div class="text-center">
            <h3 class="text-sm font-medium">No logs found</h3>
            <p
              v-if="logsStore.searchTerm || logsStore.selectedLogType !== 'ALL'"
        ...
        <div v-else class="h-[300px]">
          <div class="max-h-full overflow-x-auto">
            <table class="table-pin-rows table-compact table w-full text-sm">
              ...
              <tbody>
                <tr
                  v-for="(entry, index) in logsStore.paginatedEntries"
                  :key="index"
                  class="hover:bg-base-200"
                >
                  <td class="text-center">
                    {{ (logsStore.currentPage - 1) * logsStore.itemsPerPage + index + 1 }}
                  </td>
                  <td class="text-center">
                    <span class="badge badge-xs" :class="getLogLevelClass(entry.level)">
                      {{ entry.level }}
                    </span>
                  </td>
                  <td class="max-w-xs truncate font-mono text-xs">{{ entry.message }}</td>
                  <td class="text-center">
      ...
      <div class="bg-base-300 border-base-300 flex items-center justify-between border-t p-4">
        <div class="text-xs">
          {{ logsStore.selectedLogFile?.name }} | {{ logsStore.logEntries.length }} entries
          <span v-if="logsStore.searchTerm || logsStore.selectedLogType !== 'ALL'">(filtered)</span>
        </div>

        <div class="flex items-center space-x-2" v-if="logsStore.totalPages > 1">
          <button
          ...
          <span class="text-sm">
            Page {{ logsStore.currentPage }} of {{ logsStore.totalPages }}
          </span>
  ...
  >
    <div class="mb-4 flex justify-between">
      <div>
        <span class="badge ml-2" :class="getLogLevelClass(selectedLogEntry.level)">
          {{ selectedLogEntry.level }}
        </span>
      </div>
    </div>
    <div class="bg-base-100 max-h-[350px] flex-1 overflow-auto rounded-md p-4">
      <pre class="font-mono text-xs whitespace-pre-wrap">{{
        selectedLogEntry.content.join('\n')
      }}</pre>
    </div>
    <template #footer>
      <button class="btn" @click="closeLogDetail">Close</button>
    </template>
  </Modal>
  ...
  >
    <div class="mb-4">
      ...
      <div class="bg-base-200 mt-2 rounded p-2">
        <p class="truncate font-mono text-xs">{{ logToDelete?.message }}</p>
      </div>
    </div>
  </Modal>
  ...
  >
    <div class="mb-4">
      ...
      <div class="bg-base-200 mt-2 rounded p-2">
        <p class="font-mono text-xs">{{ logsStore.selectedLogFile.name }}</p>
      </div>
    </div>
  </Modal>
</template>
