...
import { computed, inject, ref, toRaw, onMounted, onUnmounted } from 'vue';
...
async function analyzeFile() {
	...
	try {
		...
		const tableResult: ExtractTablesResponse =
			await window.ipcRenderer.extractTablesFromSql(
				restoreConfig.value.filePath
			);
...
function closeRestoreModal() {
	if (isRestoring.value) {
		if (
			confirm('Are you sure you want to cancel the database restoration?')
		) {
...
async function updateConnectionDatabase(
	projectId: string,
	...
	try {
		...
		const connection = connections.find(
			(conn: ProjectConnection) => conn.id === projectId
		);
		...
		if (connection) {
			...
			showAlert(
				`Connection database updated to ${targetDatabase}`,
				'success'
			);
		} else {
...
async function startRestore() {
	...
	try {
		...
		const result =
			await window.ipcRenderer.restoreDatabase(restorationConfig);
		...
		<div>
				...
				<fieldset class="fieldset mb-4 w-full">
					<label class="label">
						<span class="label-text"
							>SQL Dump File (.sql or .sql.gz)</span
						>
					</label>
			...
			<div v-if="currentStep === 1">
				...
				<div
					...
					class="text-base-content my-4 text-sm"
				>
			...
			<div v-if="currentStep === 2">
				<fieldset
					...
					class="fieldset mb-4 w-full"
				>
					<label class="label">
						<span class="label-text"
							>Tables to Ignore Records (Optional)</span
						>
					</label>
				...
				<fieldset class="fieldset mb-4 w-full">
					<div class="alert alert-info">
						...
						<div>
							...
							<p class="text-sm">
								Would you like to update your connection to use
								this new database?
							</p>
						</div>
					</div>
					...
					<label class="label">
						<span class="label-text-alt"
							>If enabled, this database will be set as the
							default for future connections</span
						>
					</label>
				</fieldset>
			</div>
