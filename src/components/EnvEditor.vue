<script setup lang="ts">
  ...
  const projectPath = computed(() => connectionsStore.getSelectedProject?.projectPath);
  const envFilePath = computed(() => (projectPath.value ? `${projectPath.value}/.env` : ''));
  ...
  async function saveEnvFile() {
    if (!envContent.value || !hasChanges.value || isSaving.value || !envFilePath.value) return;

    isSaving.value = true;
    showAlert('Saving...', 'info');

    try {
      const result = await window.ipcRenderer.saveFile(envFilePath.value, envContent.value);
...
<template>
  ...
  >
    ...
    <div v-if="!projectPath" class="flex h-64 items-center justify-center">
    ...
    <div v-else-if="!envContent && !isLoading" class="flex h-64 items-center justify-center">
      <div class="text-center">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
        ...
        <button class="btn btn-sm mt-4" @click="loadEnvFile">Retry</button>
      </div>
    </div>
