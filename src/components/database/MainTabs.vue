<script setup lang="ts">
  ...
  function handleDrop(_: DragEvent, tableName: string) {
    ...
    const targetTabIndex = openTables.value.findIndex(table => table.name === tableName);
  ...
  function checkScrollPosition() {
    ...
    const hasRightScrollSpace =
      Math.abs(container.scrollWidth - container.clientWidth - container.scrollLeft) > 2;
  ...
  async function scrollToActiveTab() {
    ...
    const activeTabElement = Array.from(tabsScrollRef.value.querySelectorAll('.tab')).find(tab =>
      (tab as HTMLElement).classList.contains('bg-base-100')
  ...
  >
    <div class="tabs-content-wrapper">
      ...
      <div ref="tabsScrollRef" class="tabs-scroll" @scroll="checkScrollPosition">
        <div
          ...
          :class="[
            'tab',
            {
              ...
              'bg-base-100 border-accent border-t-2': table.name === activeTableName,
              '': table.isPinned,
            },
          ]"
          draggable="true"
        ...
        >
          <span class="pin-indicator" @click.stop="tabsStore.toggleTablePin(table)">
            <IconPin v-if="!table.isPinned" class="opacity-20 hover:opacity-100" />
            <IconPinFilled v-else class="text-accent" />
          </span>
          <span class="tab-title">{{ table.name }}</span>
          <button class="close-icon" @click.stop="tabsStore.removeTab(table)">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
    ...
    >
      <div class="dropdown dropdown-end">
        <div tabindex="0" class="tab-action-button ml-1 flex cursor-pointer items-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="currentColor"
            viewBox="0 0 24 24"
        ...
        <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box w-56 p-2 shadow-sm">
          <li>
            <a @click="confirmTruncatePinnedTables()" class="text-error">
              <IconTruncate />
  ...
  >
    <p class="py-4">
      <span class="block font-bold"
        >Are you sure you want to truncate <strong>{{ tabsStore.pinnedTables.length }}</strong>
      </span>
    ...
    <div class="bg-base-100 mb-4 max-h-40 overflow-y-auto rounded-sm p-2">
      <ul class="list-disc space-y-1 pl-4">
        <li v-for="table in tabsStore.pinnedTables" :key="table.name">
          {{ table.name }}
        </li>
      </ul>
    </div>
