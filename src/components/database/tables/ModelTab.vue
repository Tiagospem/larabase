  ...
  async function loadModel() {
    ...
    try {
      ...
      if (projectStore.models && projectStore.models.models) {
        const matchingModel = projectStore.models.models.find(m => m.table === props.tableName);
    ...
    try {
      if (projectStore.models && projectStore.models.models) {
        const relatedModel = projectStore.models.models.find(
          m => m.name === modelName || m.fullName.endsWith(`\\${modelName}`)
        );

        if (relatedModel && relatedModel.table) {
          const table = sidebarStore.localTables.find((t: Table) => t.name === relatedModel.table);
          if (table) {
  ...
  <div class="flex h-full flex-col overflow-auto">
    <div class="border-b-base-300 bg-base-200 flex items-center justify-between border-b p-2">
      <div class="flex items-center space-x-2">
        ...
        <div v-if="modelFound" class="flex space-x-2">
          <button
            v-if="model?.path"
          ...
          <button class="btn btn-sm btn-ghost" @click="viewModelCode()" title="View Code">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
      ...
      <div v-if="modelFound" class="text-xs font-medium">
    ...
    <div class="flex-1 overflow-auto">
      ...
      <div v-else-if="!modelFound" class="flex h-full w-full items-center justify-center">
      ...
      <div v-else-if="relationships.length > 0" class="overflow-x-auto">
        <h4 class="my-2 px-2 text-sm font-medium">{{ model?.name }} Relationships</h4>
        <table class="table-pin-rows table-compact table w-full text-sm">
          ...
          <tbody>
            <tr v-for="(rel, index) in relationships" :key="index" class="hover:bg-base-200">
              <td>
                <span class="badge badge-xs" :class="getRelationshipBadgeClass(rel.type)">
                  {{ rel.type }}
                </span>
              </td>
              <td class="font-mono text-xs">
                <div class="flex items-center gap-1">
                  {{ rel.relatedModel }}
                  <button
                    @click="navigateToRelatedTable(rel.relatedModel)"
      ...
      <div v-else class="flex h-full w-full items-center justify-center">
        <div class="text-center">
          <p class="text-sm">No relationships found in {{ model?.name }}</p>
        </div>
      </div>
    </div>
    ...
    >
      <PhpViewer :code="modelContent" language="php" height="500px" />
    </Modal>
  </div>
</template>
