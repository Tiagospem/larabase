<script setup lang="ts">
  ...
  onMounted(async () => {
    try {
      ...
      const response = await dataTableStore.getTableStructure(props.tableName);
...
<template>
  <div class="flex h-full flex-col overflow-auto">
    ...
    <div class="flex-1 overflow-auto">
      ...
      <div
        v-else-if="!dataTableStore.tableStructure || tableStructure.length === 0"
        class="flex h-full w-full items-center justify-center"
      >
        <div class="text-center">
          <h3 class="text-lg font-medium">No structure data available</h3>
          <p class="text-base-content/70 mt-2">Table structure information could not be loaded</p>
        </div>
      </div>
        ...
        <table class="table-pin-rows table-compact table w-full text-sm">
          ...
          <tbody>
            ...
            >
              ...
              <td :class="column.nullable ? 'text-success' : 'text-error'">
                {{ column.nullable ? 'YES' : 'NO' }}
              </td>
              <td>
                <span v-if="column.key === 'PRI'" class="badge badge-sm">PK</span>
                <span v-else-if="column.foreign_key" class="badge badge-sm badge-secondary"
                  >FK</span
                >
                <span v-else-if="column.key === 'UNI'" class="badge badge-sm badge-accent"
                  >UNI</span
                >
                <span v-else-if="column.key === 'MUL'" class="badge badge-sm badge-accent"
                  >MUL</span
                >
              </td>
              <td class="font-mono text-xs">{{ column.default ?? 'NULL' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div
      v-if="dataTableStore.tableStructure && dataTableStore.tableStructure.length > 0"
      class="bg-base-300 border-base-300 border-t p-2 text-xs"
    >
      <span>{{ props.tableName }} | {{ dataTableStore.tableStructure.length }} columns</span>
    </div>
  </div>
</template>
