...
<template>
  ...
  >
    ...
    <span class="hidden sm:inline"
      >Delete{{
        dataTableStore.selectedRows.length > 0 ? ` (${dataTableStore.selectedRows.length})` : ''
      }}</span
    >
  </button>

  <div class="modal z-50" :class="{ 'modal-open': confirmDelete }">
    <div class="modal-box">
      <h3 class="text-error text-lg font-bold">Delete Records</h3>
      <p class="py-4">
        Are you sure you want to delete
        {{ dataTableStore.selectedRows.length }} record(s)? This action cannot be undone.
      </p>

      <fieldset class="fieldset mt-2">
        <label class="label cursor-pointer justify-start gap-2">
          <input type="checkbox" v-model="ignoreForeignKeys" class="checkbox checkbox-sm" />
          <span class="label-text">Ignore foreign key constraints (use with caution)</span>
        </label>
        <p v-if="ignoreForeignKeys" class="text-warning mt-1 text-xs">
          Warning: This may cause data inconsistency if related records exist in other tables.
        </p>
      </fieldset>

      <div class="modal-action">
        <button :disabled="databaseStore.isDeletingRows" class="btn" @click="confirmDelete = false">
          Cancel
        </button>
        <button :disabled="databaseStore.isDeletingRows" class="btn btn-error" @click="deleteRows">
          Delete
        </button>
      </div>
    </div>
    <div class="modal-backdrop" @click="confirmDelete = false" />
  </div>
</template>
