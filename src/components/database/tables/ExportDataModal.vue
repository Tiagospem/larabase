<script setup lang="ts">
  ...
  const isFiltered = computed(() => dataTableStore.filterTerm || dataTableStore.activeFilter);
  ...
  function generateCSV(data: Record<string, any>[]): string {
    ...
    const columns = dataTableStore.columns.map((column: TableColumn) => column.field);
    let csv = '';
  ...
  async function generateExcel(data: Record<string, any>[]): Promise<ArrayBuffer> {
  ...
  async function fetchAllFilteredData(): Promise<Record<string, any>[]> {
    try {
      const filter =
        ...
        (dataTableStore.filterTerm ? generateFilterFromTerm(dataTableStore.filterTerm) : null);
  ...
  function generateFilterFromTerm(term: string): string | null {
    ...
    if (columns && columns.length > 0) {
      const likeFilters = columns.map(
        (column: string) => `${column} LIKE '%${searchTerm.replace(/'/g, "''")}%'`
      );
...
<template>
  <div class="modal z-50" :class="{ 'modal-open': show }">
    <div class="modal-box bg-base-300 relative max-w-lg">
      ...
      <h3 class="mb-4 flex items-center justify-between text-lg font-bold">
        Export Data
        <button class="btn btn-sm btn-circle" @click="emit('close')" :disabled="isExporting">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-6 w-6"
            fill="none"
        ...
        <fieldset class="fieldset">
          <label class="label cursor-pointer justify-start">
            ...
            <span class="label-text ml-2"
              >Export all records (uncheck to export only current page)</span
            >
          </label>
        </fieldset>
      </div>
      ...
      <div class="modal-action">
        <button class="btn btn-ghost" @click="emit('close')" :disabled="isExporting">Cancel</button>
        <button class="btn btn-primary" @click="exportData" :disabled="isExporting">Export</button>
      </div>
    </div>
