...
const isTextColumn = (columnName: string): boolean => {
	const columnType = tableStructureHelper.getColumnType(columnName);
	return columnType
		? columnType.includes('text') || columnType.includes('longtext')
		: false;
};
...
const isBooleanColumn = (columnName: string): boolean => {
	const columnType = tableStructureHelper.getColumnType(columnName);
	return columnType
		? columnType === 'tinyint(1)' || columnType === 'boolean'
		: false;
};
...
watch(
	...
	(newRecord) => {
		if (newRecord) {
			...
			if (
				tableStructureHelper.structure &&
				tableStructureHelper.structure.value
			) {
				tableStructureHelper.structure.value.forEach((column) => {
					...
					if (
						isDateTimeColumn(column.name) &&
						formData.value[column.name]
					) {
						const columnType = tableStructureHelper.getColumnType(
							column.name
						);
						...
						if (!isNaN(date.getTime())) {
							if (columnType === 'date') {
								formData.value[column.name] = date
									.toISOString()
									.split('T')[0];
							} else {
								formData.value[column.name] = date
					...
					if (
						isJsonColumn(column.name) &&
						formData.value[column.name]
					) {
						try {
							if (
								typeof formData.value[column.name] === 'string'
							) {
								formData.value[column.name] = JSON.parse(
									formData.value[column.name]
								);
							}
		...
		if (
			...
			tableStructureHelper.structure.value.forEach((column) => {
				if (isJsonColumn(column.name)) {
					if (processedFormData[`${column.name}_raw`]) {
						try {
							...
							console.error(
								`Error parsing JSON for ${column.name}:`,
								e
							);
						}
					}
					...
					if (processedFormData[column.name] === null) {
						...
						processedFormData[column.name] = Boolean(
							processedFormData[column.name]
						);
					}
				} else if (
				...
				if (
					...
					try {
						processedFormData[column.name] = JSON.stringify(
							processedFormData[column.name]
						);
					} catch (e) {
						...
						<label class="label mb-1">
							<span class="label-text font-medium">
								...
								<span
									...
									class="text-warning ml-1 text-xs"
									>(Primary Key)</span
								>
								<span
									v-else-if="column.isForeignKey"
									class="text-info ml-1 text-xs"
									>(Foreign Key)</span
								>
							</span>
							<span
