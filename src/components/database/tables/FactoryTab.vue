...
const debouncedSetAttributeValue = debounce(
...
async function executeSeedCommand() {
	...
	try {
		...
		if (!success) {
			showAlert(
				'Failed to seed data, check terminal for details',
				'error'
			);
		}
	} catch (error: any) {
...
<template>
	<div class="flex h-full flex-col overflow-auto">
		...
		<div class="flex-1 overflow-auto">
			...
			>
				<div class="text-center">
					...
					<p class="text-base-content mt-2 text-xs">
						Factory files are typically located in
			...
			<div
				v-else-if="
					...
					factoryStore.factory.attributes?.length > 0
				"
			...
			<div
				v-else-if="factoryStore.factory"
				class="flex h-full w-full items-center justify-center"
			>
				<div class="text-center">
					<p class="text-sm">
						No factory attributes were detected for
						{{ factoryStore.modelName }}
					</p>
					<p class="text-base-content mt-2 text-xs">
		...
		>
			<span
				>{{ props.tableName }} |
				{{ factoryStore.factory.attributes?.length || 0 }} factory
				attributes</span
			>
		</div>
		...
		>
			<PhpViewer
				:code="factoryStore.factory.content"
				language="php"
				height="500px"
			/>
		</Modal>
			...
			<div class="space-y-4">
					...
					<fieldset class="fieldset">
						<div class="label">
							<span class="label-text"
								>Number of records to create</span
							>
						</div>
						<input
							type="number"
				...
				<div v-if="factoryStore.factory.attributes?.length > 0">
					<fieldset class="fieldset">
						<div class="label">
							<span class="label-text"
								>Override attributes (optional)</span
							>
						</div>
						<div class="max-h-60 overflow-y-auto">
							<table
								class="table-compact table w-full table-fixed"
							>
								<colgroup>
									<col class="w-14" />
									<col class="w-1/4" />
								...
								<thead
									class="bg-base-200 sticky top-0 z-10 shadow-md"
								>
									<tr>
								...
								<tbody>
									<tr
										v-for="attr in factoryStore.factory
											.attributes"
										:key="attr.name"
									...
									>
										<td class="text-center align-middle">
											<input
												...
												@change="
													() =>
														factoryStore.toggleAttributeSelection(
															attr.name
														)
												"
											/>
										...
										<td
											class="align-middle font-mono text-xs"
										>
											{{ attr.name }}
										</td>
										<td
											class="truncate align-middle font-mono text-xs"
											:title="attr.value"
										>
											{{ formatFactoryValue(attr.value) }}
										</td>
				...
				<div class="bg-base-300 rounded-lg p-3">
					<fieldset class="fieldset">
						...
						<pre
							class="bg-base-100 overflow-x-auto rounded-lg p-2 font-mono text-xs"
							>{{ factoryStore.generateSeedCommand }}</pre
						>
					</fieldset>
				</div>
			</div>
