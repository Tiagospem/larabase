<script setup lang="ts">
  ...
  async function loadTableIndexes() {
    try {
      ...
      const response = await window.ipcRenderer.getTableIndexes(dbConfig, props.tableName);

      if (response.success) {
        tableIndexes.value = response.indexes;
      } else {
        showAlert(`Failed to load table indexes: ${response.message}`, 'error');
      }
    } catch (error) {
...
<template>
  <div class="flex h-full flex-col overflow-auto">
    <div class="border-b-base-300 bg-base-200 flex items-center justify-between border-b p-2">
      <button class="btn btn-sm btn-ghost" @click="loadTableIndexes">
        <svg
    ...
    <div class="flex-1 overflow-auto">
      ...
      >
        <div class="text-center">
          ...
          <p class="text-base-content/70 mt-2">This table doesn't have any indexes</p>
        </div>
      </div>
      ...
      <div v-else class="overflow-x-auto">
        <table class="table-pin-rows table-compact table w-full text-sm">
          ...
          <tbody>
            <tr v-for="(index, i) in tableIndexes" :key="index.name" class="hover:bg-base-200">
              <td class="text-right">{{ i + 1 }}</td>
              <td class="font-mono text-xs">{{ index.name }}</td>
              <td>
                <span :class="['badge badge-xs', getBadgeClass(index.type)]">{{ index.type }}</span>
              </td>
              <td class="font-mono text-xs">{{ index.columns.join(', ') }}</td>
              <td class="font-mono text-xs">{{ index.algorithm }}</td>
              <td class="font-mono text-xs">{{ index.cardinality }}</td>
              <td class="font-mono text-xs">{{ index.comment || '-' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    >
      <span>{{ props.tableName }} | {{ tableIndexes.length }} indexes</span>
    </div>
  </div>
</template>
