...
async function loadForeignKeys() {
	try {
		...
		foreignKeys.value = await dataTableStore.getTableForeignKeys(
			props.tableName
		);
	} catch (error: any) {
		showAlert(
			`Failed to load foreign keys: ${error.message || error}`,
			'error'
		);
	} finally {
...
<template>
	<div class="flex h-full flex-col overflow-auto">
		...
		<div class="flex-1 overflow-auto">
			...
			>
				<div v-if="outgoingRelations.length > 0">
					<h4 class="bg-base-100 px-2 py-1 font-medium">
						This Table → Other Tables
					</h4>
				...
				<div
					...
					class="mt-4"
				>
					<h4 class="text-md bg-base-100 px-2 py-1 font-medium">
		...
		>
			<span
				>{{ props.tableName }} |
				{{ foreignKeys.length }} relationships</span
			>
		</div>
	</div>
</template>
