			...
			<div class="flex flex-col gap-6 px-1">
				...
				>
					<div class="mb-2 flex items-center justify-between">
						...
						<span
							class="badge badge-xs badge-base-200 text-xs opacity-70"
							>{{ getColumnTypeLabel(column.name) }}</span
						>
					</div>

					<div
						v-if="isJsonColumn(column.name)"
						class="mt-2"
					>
						<div
							v-if="props.record[column.name]"
							class="relative"
						>
							<pre
							...
							<button
								v-if="
									isJsonTruncated(props.record[column.name])
								"
					...
					<div
						v-else-if="isTextColumn(column.name)"
						class="mt-2"
					>
						<div
							v-if="props.record[column.name]"
						...
						>
							<div
								v-if="
									containsHtmlTags(
										props.record[column.name]
									) && htmlPreviewMode.has(column.name)
								"
					...
					<div
						v-else-if="isBooleanColumn(column.name)"
						class="mt-2"
					>
						<div
					...
					<div
						v-else-if="isDateTimeColumn(column.name)"
					...
					>
						<span
							v-if="
								props.record[column.name] !== null &&
								props.record[column.name] !== undefined
							"
							class="text-sm"
						>
							{{ props.record[column.name] }}
