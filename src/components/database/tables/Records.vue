<script setup lang="ts">
  ...
  const handleSort = (field: string) => {
    ...
    dataTableStore.setSortDirection(dataTableStore.currentSortDirection === 'asc' ? 'desc' : 'asc');
    handleRefresh();
  };
...
<template>
  ...
  >
    <div class="flex flex-wrap items-center gap-0.5">
      ...
      <TruncateButton @refresh="handleRefresh" :table-name="props.tableName" />
      <DeleteButton @refresh="handleRefresh" :table-name="props.tableName" />
      <LiveTableButton @refresh="handleRefresh" />
    </div>

    <div class="flex flex-wrap items-center gap-2">
      <FilterButton @refresh="handleRefresh" :table-name="props.tableName" />
    </div>
  </div>
