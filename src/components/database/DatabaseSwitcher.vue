<script setup lang="ts">
  ...
  async function switchDatabase(databaseName: string, shouldUpdateEnv: boolean) {
    ...
    try {
      if (shouldUpdateEnv && project.value.projectPath && databaseName !== projectDatabase.value) {
      ...
      if (databaseName !== project.value.db_config.database) {
        ...
        await connectionStore.updateConnection(project.value.id as string, project.value);
  ...
  async function loadAvailableDatabases() {
    ...
    try {
      const result = await window.ipcRenderer.listDatabases(toRaw(project.value.db_config));
...
<template>
  <div>
    ...
    >
      ...
      <div v-else-if="availableDatabases.length === 0" class="py-4 text-center">
        No databases found
      </div>
      <div v-else>
        <div class="mb-3 flex items-center justify-between">
          <div class="flex items-center gap-2 text-sm">
            <div>
              <span class="badge badge-sm badge-primary mr-1">CURRENT</span> Current connection
            </div>
            <div v-if="project?.projectPath">
              <span class="badge badge-sm badge-accent mr-1">ENV</span> Project .env file
            </div>
          </div>
          <button class="btn btn-xs btn-neutral" @click="showCreateDatabaseForm = true">
            <svg
        ...
        <div v-if="showCreateDatabaseForm" class="bg-base-100 mb-4 rounded-md p-3">
          <fieldset class="fieldset w-full">
            ...
            <div class="flex gap-2">
              ...
              >
                <span v-if="isCreatingDatabase" class="loading loading-spinner loading-xs"></span>
                <span v-else>Create</span>
              </button>
              <button class="btn btn-sm btn-ghost" @click="showCreateDatabaseForm = false">
                Cancel
              </button>
            </div>
            <label v-if="databaseNameError" class="label">
        ...
        <div class="mb-4 max-h-80 overflow-x-hidden overflow-y-auto">
          <div class="grid gap-2">
            <div
              ...
              :class="{
                'bg-base-200': db === project?.db_config?.database,
                'border-l-accent border-l-4': db === projectDatabase,
              }"
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center">
                  ...
                  <span
                    v-if="db === projectDatabase && db !== project?.db_config?.database"
                    class="badge badge-sm badge-accent ml-2"
                    >ENV</span
                  >
                  <span
                    v-if="db === project?.db_config?.database && db === projectDatabase"
                    class="badge badge-sm badge-success ml-2"
                    >ENV + CURRENT</span
                  >
                  <span
                    v-else-if="db === project?.db_config?.database"
                ...
                <div class="flex gap-3">
                  <div
                    v-if="db !== project?.db_config?.database"
                  ...
                  <div
                    v-if="
                      project?.projectPath &&
                      (db !== projectDatabase || db !== project?.db_config?.database)
                    "
                    class="tooltip tooltip-left"
