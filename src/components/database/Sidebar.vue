<script setup lang="ts">
  ...
  function toggleTableSelection(table: Table) {
    if (isTableSelected(table.name)) {
      selectedTables.value = selectedTables.value.filter(t => t.name !== table.name);
    } else {
  ...
  async function dropTables() {
    ...
    try {
      const params = {
        ...
        dbConnection: toRaw(selectedProject.value?.db_config) as MysqlConnection,
...
<template>
  <div class="relative z-10 flex h-full flex-col">
    <div class="bg-base-200 flex h-full w-full flex-col border-r border-black/10">
      <div class="shrink-0 border-b border-black/10 p-3">
        ...
        <div class="flex items-center justify-between">
          <div class="truncate text-xs">{{ sidebarStore.filteredTables.length }} tables</div>
          <div class="flex items-center gap-1">
            <div class="tooltip tooltip-right" data-tip="Sort by name">
              <button
                class="btn btn-xs btn-ghost"
                :class="{
                  'bg-base-100': sidebarStore.sortBy === 'name',
                }"
            ...
            <div class="tooltip tooltip-right" data-tip="Sort by records">
              <button
                class="btn btn-xs btn-ghost"
                :class="{
                  'bg-base-100': sidebarStore.sortBy === 'records',
                }"
            ...
            <div class="tooltip tooltip-right" data-tip="Toggle sort order">
              <button class="btn btn-xs btn-ghost" @click="sidebarStore.toggleSortOrder">
                <svg
        ...
        <div v-if="isDeleteMode" class="mt-2 flex items-center justify-between text-sm">
      ...
      <div class="min-h-0 flex-1 overflow-x-hidden overflow-y-auto">
        <div v-if="sidebarStore.isLoading" class="p-2">
        ...
        <ul v-else class="w-full">
          ...
          >
            <a
              @click="isDeleteMode ? toggleTableSelection(table) : tabsStore.openTable(table)"
            ...
            >
              <div v-if="isDeleteMode" class="flex-shrink-0">
                <input
                  type="checkbox"
              ...
              <div class="flex min-w-0 flex-1 flex-col">
                <span class="truncate text-sm font-semibold">{{ table.name }}</span>
                <span v-if="getTableModel(table.name)" class="truncate text-xs opacity-60">
                  {{ getTableModel(table.name)?.namespace }}\{{ getTableModel(table.name)?.name }}
                </span>
              </div>
              ...
              <span
                ...
                :class="{
                  ...
                  'badge-accent': table.rowCount > 0 || table.isApproximate,
                }"
              >
                {{ sidebarStore.formatRecordCount(table.rowCount) }}
              </span>
            </a>
          </li>
        </ul>
    ...
    <div class="mt-2 text-sm">
      ...
      <div class="bg-base-200 max-h-32 overflow-y-auto rounded-sm p-2">
        <ul class="list-disc space-y-1 pl-4">
          <li v-for="table in selectedTables" :key="table.name">
            {{ table.name }}
          </li>
        </ul>
      </div>
