...
const filteredTables = computed(() => {
	if (!props.schemaData?.tables || !Array.isArray(props.schemaData.tables)) {
		console.warn(
			'No tables found in schema data or tables is not an array'
		);
		return [];
	}
...
function toggleExpandTable(tableName: string) {
	if (expandedTables.value.includes(tableName)) {
		expandedTables.value = expandedTables.value.filter(
			(name) => name !== tableName
		);
	} else {
...
function expandAllTables() {
	if (props.schemaData?.tables) {
		expandedTables.value = props.schemaData.tables.map(
			(table: TableSchema) => table.name
		);
	}
}
				...
				>
					...
					>
						<div class="flex items-center gap-2">
							<svg
								...
								:class="{
									'rotate-90': expandedTables.includes(
										table.name
									)
								}"
								fill="none"
					...
					>
						<div class="overflow-x-auto">
							<table
								class="table-sm table-zebra table w-full text-xs"
							>
								<thead>
									<tr>
										<th>Column</th>
								...
								<tbody>
									...
									>
										...
										<td :class="getTypeClass(column.type)">
