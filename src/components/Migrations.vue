<script setup lang="ts">
  ...
  async function viewMigrationCode(migrationName: string) {
    ...
    try {
      ...
      if (result.success) {
        ...
        showAlert(`Failed to read migration file: ${result.error}`, 'error');
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred';
  ...
  async function runMigrations() {
    ...
    try {
      ...
      const command = usingSail ? 'vendor/bin/sail artisan migrate' : 'php artisan migrate';
  ...
  onMounted(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        errorMessage.value = result.message || 'Failed to retrieve migration status.';
      }
    } catch (error) {
...
<template>
  ...
  >
    <div>
      <div class="mb-4 flex justify-end gap-2">
        ...
        >
          <span v-if="isRunning" class="loading loading-spinner loading-xs mr-1"></span>
          <svg
            v-else
      ...
      <div v-else-if="errorMessage" class="alert alert-error mb-2 text-sm">
        <div>
          <svg
            xmlns="http://www.w3.org/2000/svg"
      ...
      <div v-else>
        <div>
          <div class="mb-2 flex items-center justify-between">
            <h3 class="text-xs font-semibold uppercase">Pending Migrations</h3>
            <span class="badge badge-sm">{{ pendingMigrations.length }}</span>
          </div>
          ...
          >
            ...
            <p class="text-xs opacity-70">No pending migrations found.</p>
          </div>

          <div v-else class="border-base-300 mb-4 max-h-[200px] overflow-y-auto rounded-lg border">
            <table class="table-pin-rows table-compact table w-full text-sm">
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="font-mono text-xs">{{ migration }}</td>
          ...
          <div class="mt-4">
            <div class="mb-2 flex items-center justify-between">
              <h3 class="text-xs font-semibold uppercase">Applied Migrations</h3>
              <span class="badge badge-sm">{{ migrationsHistory.length }}</span>
            </div>
            ...
            <div
              ...
              class="bg-base-200 rounded-box p-3 text-center"
            >
              <p class="text-xs opacity-70">No applied migrations found.</p>
            </div>

            <div v-else class="border-base-300 max-h-[200px] overflow-y-auto rounded-lg border">
              <table class="table-pin-rows table-compact table w-full text-sm">
                <thead class="bg-base-100 sticky top-0 z-10 shadow-md">
                  <tr>
                    <th class="w-12 text-center">#</th>
                ...
                <tbody>
                  <tr
                    v-for="(migration, index) in migrationsHistory"
                    :key="migration"
                    class="hover:bg-primary hover:text-base-100 bg-base-200"
                  >
                    <td class="text-center">{{ index + 1 }}</td>
  ...
  >
    <div class="h-[70vh] overflow-auto">
      <PhpViewer :code="selectedMigration.code" language="php" height="100%" />
    </div>
  </Modal>
  ...
  >
    <div class="p-2">
      <div class="bg-base-200 mb-4 rounded-lg p-3">
        <div class="mb-2">
          <span class="text-sm font-medium">Select number of migrations to rollback</span>
        </div>
        <input
          type="range"
          min="0"
        ...
        <div class="label mt-1">
          <span class="label-text-alt">
            ...
            <code class="bg-base-300 rounded px-2 py-1 font-mono text-xs">
              {{ selectedProject?.usingSail ? 'sail' : 'php' }} artisan migrate:rollback --step={{
                rollbackStep
              }}
            </code>
          </span>
      ...
      >
        <div class="bg-base-200 border-base-300 border-b px-3 py-2">
          <h3 class="text-xs font-semibold uppercase">
            Applied Migrations to Roll Back ({{ migrationsToRollback.length }})
          </h3>
        </div>
        <div class="flex-1 overflow-auto">
          <table class="table-compact table w-full text-sm">
            ...
            <tbody>
              <tr
                v-for="(migration, index) in migrationsToRollback"
                :key="migration"
                class="hover:bg-primary hover:text-base-100 bg-base-200"
              >
                <td class="text-center">{{ index + 1 }}</td>
      ...
      >
        <div>
          ...
          <p class="text-sm font-medium">Move the slider to select migrations to roll back</p>
          <p class="mt-1 text-xs opacity-70">Select at least one migration to enable rollback</p>
        </div>
      </div>
    </div>
  </Modal>
