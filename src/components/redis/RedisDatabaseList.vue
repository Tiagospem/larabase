<script setup lang="ts">
  ...
  async function flushDatabase(dbId: number, event: Event) {
    ...
    if (!confirm(`Are you sure you want to flush database ${dbId}? This will delete all keys.`)) {
      return;
    }
...
<template>
  <div class="flex h-full flex-col">
    <div v-if="redisStore.isLoadingDbs" class="flex h-full w-full items-center justify-center">
      ...
      >
        <div class="card-body p-3">
          <div class="flex items-center justify-between">
            ...
            >
              <span v-if="flushingDb === db.id" class="loading loading-spinner loading-xs"></span>
          ...
          <div class="text-sm">
            <div class="mt-1 grid grid-cols-2 gap-2">
              <div><span class="font-semibold">Keys:</span> {{ db.keys }}</div>
              <div><span class="font-semibold">Expires:</span> {{ db.expires }}</div>
            </div>

            <div v-if="db.avgTtl > 0" class="mt-1">
              <span class="font-semibold">Avg TTL:</span> {{ Math.round(db.avgTtl / 1000) }}s
            </div>
          </div>
        </div>
      </div>
