...
<template>
  <div class="flex h-full flex-col">
    <h3 class="mb-3 px-1 text-sm font-semibold">
      Keys <span v-if="redisStore.keys.length > 0">({{ redisStore.keys.length }})</span>
    </h3>
    ...
    <div v-else class="flex-1 overflow-y-auto">
      <div class="space-y-1 px-1">
        <div
          ...
          :class="{ 'border-primary': redisStore.currentKeyInfo?.key === key.key }"
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center overflow-hidden">
              <span
                ...
                >{{ key.type }}</span
              >
              <span class="truncate font-mono text-xs" :title="key.key">{{ key.key }}</span>
            </div>
            <button
      ...
      <div
        ...
        class="mt-4 flex justify-center"
      >
        <button @click="loadMoreKeys" class="btn btn-sm btn-outline">Load more</button>
      </div>
    </div>
  </div>
</template>
