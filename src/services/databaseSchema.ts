...
export const useDatabaseSchema = () => {
  ...
  const fetchDatabaseSchema = async (force = false): Promise<DatabaseSchema | null> => {
    ...
    try {
      ...
      if (result.success) {
        const modelsResult = await projectStore.getModelsForTables(selectedProject);
  ...
  const getCompactSchema = (): string => {
    ...
    const tableStrings = databaseSchema.value.tables.map(table => {
      ...
      if (table.columns && table.columns.length > 0) {
        const columnStrings = table.columns.map(col => `C:${col.name}:${col.type}`);
      ...
      if (table.foreignKeys && table.foreignKeys.length > 0) {
        const fkStrings = table.foreignKeys
          ...
          .map(fk => `FK:${fk.column}:${fk.referenced_table || ''}:${fk.referenced_column}`);
