...
export class AIService {
  ...
  private initializeClients() {
    ...
    if (settings.aiProvider === 'gemini' && settings.gemini.apiKey) {
      this.googleAIClient = new GoogleGenerativeAI(settings.gemini.apiKey);
    } else {
  ...
  public async generateSQLFromPrompt(
    ...
    try {
      if (settings.aiProvider === 'openai' && this.openaiClient) {
        return await this.generateWithOpenAI(systemPrompt, prompt, settings.openai.model);
      } else if (settings.aiProvider === 'gemini' && this.googleAIClient) {
        return await this.generateWithGemini(systemPrompt, prompt, settings.gemini.model);
      } else {
        return {
          content: '',
  ...
  public async analyzeSQLExplain(
    ...
    try {
      if (settings.aiProvider === 'openai' && this.openaiClient) {
        return await this.generateWithOpenAI(systemPrompt, explainData, settings.openai.model);
      } else if (settings.aiProvider === 'gemini' && this.googleAIClient) {
        return await this.generateWithGemini(systemPrompt, explainData, settings.gemini.model);
      } else {
        return {
          content: '',
          error: 'No AI provider configured. Please check your settings.',
  ...
  public async fixSQLQuery(sqlQuery: string, language: string = 'en'): Promise<AIResponse> {
    this.initializeClients();
    const settings = this.settingsStore.settings;
  ...
  private createSystemPrompt(databaseSchema: string, language: string): string {
