<template>
  <div class="query-optimization-tips">
    ...
    <div v-else-if="tips.length === 0" class="text-center py-4 text-gray-500">
    ...
    <div v-else class="space-y-4">
      ...
      >
        <div class="card-body p-4">
          ...
          <div v-if="tip.code" class="bg-base-300 p-3 rounded font-mono text-sm mb-2">
            {{ tip.code }}
          </div>
          
          <div v-if="tip.alternatives && tip.alternatives.length > 0" class="mt-2">
          ...
          <div v-if="tip.implementation" class="mt-3 flex justify-end">
            <button 
...
watch([() => props.explainData, () => props.query], () => {
  ...
  for (const row of data) {
    if (row.type === 'ALL') {
      tips.value.push({
        ...
        alternatives: [
          'Add an index that covers columns used in the WHERE clause',
          'Limit the number of rows returned if possible',
          'Consider using a more selective condition'
        ]
      });
    }
    ...
    if (row.Extra?.includes('Using filesort')) {
      tips.value.push({
        ...
        alternatives: [
          'Create an index that includes both WHERE and ORDER BY columns',
          'Consider if the sort is necessary or can be done at application level'
        ]
      });
    }
    ...
    if (row.type === 'ALL' && row.Extra?.includes('Using join buffer')) {
      tips.value.push({
        ...
        alternatives: [
          ...
          'Restructure the query to use more efficient join types'
        ]
      });
    }
...
function analyzeQueryStructure() {
  ...
  if (joinCount > 2) {
    tips.value.push({
      ...
      alternatives: [
        'Consider breaking down into multiple simpler queries',
        'Ensure all join columns are properly indexed',
        'Evaluate if all joins are necessary'
      ]
    });
  }
  ...
  if (query.includes('like') && query.includes('%')) {
    ...
    if (hasLeadingWildcard) {
      tips.value.push({
        ...
        alternatives: [
          'Restructure query to avoid leading wildcards',
          'Consider using full-text search for better performance',
          'If possible, use LIKE "text%" instead of LIKE "%text"'
        ]
      });
    }
  }
