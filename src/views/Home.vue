<script setup lang="ts">
  ...
  const manageConnection = ref<InstanceType<typeof ManageConnection> | null>(null);
...
<template>
  <div class="relative flex h-full">
    <div class="bg-base-300 flex w-3/12 flex-col items-center justify-between p-6">
      <div class="mt-12 flex w-full flex-col items-center text-center">
        <img src="../../public/icons/png/512x512.png" alt="Larabase" class="draggable mb-2 h-22" />
        <h1 class="text-2xl font-bold">Larabase</h1>
        <p class="mt-2 text-xs">An opinionated database GUI for Laravel developers</p>
      </div>

      <div class="mt-8 flex w-full items-center gap-2">
        <button class="btn flex flex-1 items-center gap-2" @click="openCreateConnectionModal">
          <svg
        ...
        <button class="btn btn-square" @click="showSettings = true">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
    ...
    <div class="flex w-9/12 flex-col">
      <div class="flex-1 overflow-auto p-6">
        ...
        <div v-if="isLoading" class="flex h-[calc(100vh-150px)] items-center justify-center">
        ...
        >
          ...
          <p class="mt-1 text-sm font-light">Create a new connection to get started</p>
        </div>
          ...
          >
            <div class="card-body px-5 py-4">
              <div class="flex flex-col items-start gap-4 sm:flex-row sm:items-center">
                ...
                >
                  <span class="text-base-100 font-bold">{{ connection.icon }}</span>
                </div>
                <div class="min-w-0 flex-1">
                  <h2 class="card-title overflow-hidden text-ellipsis whitespace-nowrap">
                    <span>{{ connection.name }}</span>
                    <span v-if="connection.isValid" class="text-success ml-1 text-xs">{{
                      connection.status
                    }}</span>
                    <span v-else class="text-error ml-1 text-xs">Invalid</span>
                  </h2>
                ...
                <div class="mt-2 flex gap-2 self-end sm:mt-0 sm:self-center">
                  <div class="tooltip tooltip-bottom" data-tip="Edit connection">
                    <button class="btn btn-sm btn-ghost" @click.stop="editConnection(connection)">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
                  ...
                  <div class="tooltip tooltip-bottom" data-tip="Restore database">
                    <button
                  ...
                  <div class="tooltip tooltip-bottom" data-tip="Remove connection">
                    <button
                      class="btn btn-sm btn-ghost"
                      @click.stop="removeConnection(connection.id as string)"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                  ...
                  <div class="tooltip tooltip-bottom" data-tip="Open connection">
                    <button
                      :disabled="!connection.isValid"
                      class="btn btn-sm btn-ghost"
                      @click.stop="openConnection(connection.id as string)"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
