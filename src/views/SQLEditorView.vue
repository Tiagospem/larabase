<script setup lang="ts">
  ...
  const { databaseSchema, fetchDatabaseSchema, initializeSchema, getCompactSchema } =
    useDatabaseSchema();
        ...
        if (
          ...
          try {
            ...
            if (!isNaN(date.getTime())) {
              formattedRow[key] = date.toISOString().replace('T', ' ').split('.')[0];
            }
          } catch (e) {}
        }
      });
  ...
  function loadSavedSql() {
    const savedSql = localStorage.getItem(`sql-editor-content-${projectId.value}`);
    if (savedSql) {
  ...
  function saveSqlContent() {
    localStorage.setItem(`sql-editor-content-${projectId.value}`, sqlQuery.value);

    if (sqlScratchService.activeScratchId) {
      sqlScratchService.updateActiveScratchContent(sqlQuery.value, projectId.value);
    }
  }
  ...
  function updateSectionHeights(editorRatio: number) {
    ...
    const resultHeight = containerHeight - editorHeight - resizer.value!.clientHeight;
  ...
  function onResizerMouseDown(event: MouseEvent) {
    ...
    const onMouseMove = (e: MouseEvent) => {
      if (isResizing.value) {
        ...
        if (
          newEditorHeight > 100 &&
          containerHeight - newEditorHeight - resizer.value!.clientHeight > 100
        ) {
          ...
          if (sqlEditorRef.value && typeof sqlEditorRef.value.layout === 'function') {
    ...
    const onMouseUp = () => {
      if (isResizing.value) {
        ...
        const newRatio = editorSection.value!.clientHeight / containerHeight;
  ...
  onMounted(async () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        const editorElement = document.querySelector('.sql-editor-container');
        if (editorElement) {
          editorElement.addEventListener('save-as-scratch', (event: Event) => {
  ...
  function initResize() {
    if (!editorContainer.value || !editorSection.value || !resultSection.value || !resizer.value)
      return;
...
<template>
  <div class="relative flex h-full flex-col">
    ...
    <div v-if="isLoading" class="mt-8 flex h-full items-center justify-center">
    ...
    <template v-else>
      ...
      >
        <template #actions>
          <div class="flex items-center gap-2">
            ...
            <div class="tooltip tooltip-left" data-tip="Database Schema">
              <button
            ...
            <div class="tooltip tooltip-left" data-tip="AI SQL Assistant">
              <button
                @click="openAIModal"
                class="btn btn-ghost btn-sm"
            ...
            <div class="tooltip tooltip-left" data-tip="Open Scratches">
              <button
                @click="toggleScratchPanel"
                class="btn btn-ghost btn-sm"
                title="SQL Scratches"
                :class="{ 'btn-active': sqlScratchService.isOpen }"
              >
                <svg
                  fill="currentColor"
                  class="h-4 w-4"
            ...
            <div class="tooltip tooltip-left" data-tip="Save as Scratch">
              <button
      ...
      <div class="flex h-[calc(100%-4rem)] w-full overflow-hidden">
        <div ref="editorContainer" class="flex h-full w-full flex-col overflow-hidden">
          <div ref="editorSection" class="bg-base-200 w-full">
            <SQLEditor
              ref="sqlEditorRef"
              v-model="sqlQuery"
              class="h-full w-full"
              @explain-sql="handleExplainSQL"
              @processing-state="state => (isProcessing = state)"
            />
          </div>
          ...
          >
            <div class="bg-base-content h-1 w-32 rounded-full opacity-50"></div>
          </div>

          <div ref="resultSection" class="bg-base-100 w-full overflow-auto">
            <div
            ...
            >
              ...
              <div class="bg-base-100 border-base-300 mt-3 rounded-md border p-3">
                <pre class="text-error font-mono text-sm whitespace-pre-wrap">{{
                  sqlResultsStore.error
                }}</pre>
              </div>
            </div>

            <div v-else-if="sqlResultsStore.results.length > 0" class="flex h-full w-full flex-col">
              <div class="flex-1 overflow-auto">
                <div
                  v-if="
                    sqlResultsStore.results.length === 1 &&
                    'affectedRows' in sqlResultsStore.results[0]
                  "
                  class="bg-base-200 border-base-300 m-4 rounded-md border p-4 shadow"
                >
                  <div class="mb-2 flex items-center">
                    ...
                    <h3 class="text-lg font-bold">Query Executed Successfully</h3>
                  </div>

                  <div class="mt-3 grid grid-cols-2 gap-4">
                    <div class="bg-base-100 border-base-300 rounded-md border p-3">
                      <div class="text-base-content/60 mb-1 text-sm">Affected Rows</div>
                      <div class="text-xl font-semibold">
                        {{ sqlResultsStore.results[0].affectedRows }}
                      </div>
                    </div>

                    <div
                      v-if="'changedRows' in sqlResultsStore.results[0]"
                      class="bg-base-100 border-base-300 rounded-md border p-3"
                    >
                      <div class="text-base-content/60 mb-1 text-sm">Changed Rows</div>
                      <div class="text-xl font-semibold">
                        {{ sqlResultsStore.results[0].changedRows }}
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  v-else-if="
                    sqlResultsStore.columns.length > 0 && sqlResultsStore.results.length === 0
                  "
                  class="bg-base-200 border-base-300 m-4 rounded-md border p-4 shadow"
                >
                  <div class="mb-2 flex items-center">
                  ...
                  <p class="mt-2">The query executed successfully but returned no results.</p>
                  <p class="mt-1 text-sm opacity-70">
                    Your query ran without errors, but no records matched the criteria.
                  </p>
                </div>
                <DataTable
                  v-else
            ...
            <div
              v-else-if="sqlResultsStore.success && sqlResultsStore.results.length === 0"
              class="bg-base-200 border-base-300 m-4 rounded-md border p-4 shadow"
            >
              <div class="mb-2 flex items-center">
                <svg
              ...
              <div v-if="sqlResultsStore.lastQueryTime" class="text-base-content/60 mt-3 text-xs">
                Query time: {{ sqlResultsStore.lastQueryTime }}ms
              </div>
            </div>
            ...
            >
              <p class="text-sm opacity-50">Execute a query to see results</p>
            </div>
          </div>
        </div>

        <transition name="slide">
          <SQLScratchSidebar v-if="sqlScratchService.isOpen" :projectId="projectId" />
        </transition>
      </div>
